				...
				<view v-if="currentStep === 1" class="step-content">
					...
					<view class="wechat-btn" @click="handleWechatAuth">
						<view class="wechat-btn__icon">
							<AppIcon name="wechat" :size="44" color="#FFFFFF" />
						</view>
	...
	methods: {
		...
		async handleComplete() {
			...
			try {
				...
				// 写入全局 store
				store.login('mock_token_' + Date.now(), data.userInfo);
...
/* ========== 顶部 Hero ========== */
.login-hero {
	background: linear-gradient(160deg, #4F6EF7 0%, #3D56D4 40%, #2B41B8 100%);
...
.login-card {
	...
	border-radius: 28rpx;
	padding: 50rpx 36rpx;
	box-shadow: 0 8rpx 40rpx rgba(79, 110, 247, 0.12);
	min-height: 500rpx;
}
