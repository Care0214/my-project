				...
				<scroll-view scroll-x class="recommend-scroll">
					<view class="recommend-item" v-for="item in recommendList" :key="item.id" @click="goDetail(item)">
						<view class="rec-image">
							<view class="rec-image-placeholder">
					...
					<view class="item-info">
						...
						<view class="item-footer">
							<view class="item-seller">
								<view class="item-avatar-sm" :style="{ background: item.seller && item.seller.avatarBg ? item.seller.avatarBg : '#E0E0E0' }"></view>
							...
							<view class="item-meta">
								...
								<text class="item-location" v-if="item.location || item.campus">{{ item.location || item.campus }}</text>
...
export default {
	...
	methods: {
		...
		goCategory(cat) {
			uni.navigateTo({ url: '/pages/search/index?category=' + (cat.key || cat.id) });
		},
		goDetail(item) {
			// 保存浏览记录
...
/* ======== 物品Feed列表 ======== */
.item-card {
	background: #FFF; border-radius: 20rpx; margin-bottom: 20rpx; overflow: hidden;
	box-shadow: 0 2rpx 12rpx rgba(0, 0, 0, 0.04);
}
.item-card:active { transform: scale(0.99); }
.item-image { width: 100%; height: 340rpx; position: relative; overflow: hidden; }
...
.item-price-tag {
	position: absolute; right: 20rpx; bottom: 20rpx; padding: 6rpx 20rpx;
	border-radius: 20rpx; background: rgba(0, 0, 0, 0.6); z-index: 2;
}
.price-num { font-size: 30rpx; font-weight: 700; color: #FF6B3D; }
.price-free { font-size: 26rpx; font-weight: 600; color: #22C55E; }
.item-type-tag {
	position: absolute; left: 20rpx; top: 20rpx; padding: 4rpx 16rpx;
	border-radius: 12rpx; background: rgba(34, 197, 94, 0.85); z-index: 2;
}
.item-type-tag text { font-size: 22rpx; color: #FFF; font-weight: 500; }
.tag-hot-corner {
	position: absolute; top: 0; left: 0; padding: 6rpx 20rpx;
	background: #FF6B3D; color: #FFF; font-size: 22rpx; font-weight: bold; border-radius: 0 0 12rpx 0;
}
.tag-new-corner {
	position: absolute; top: 0; left: 0; padding: 6rpx 20rpx;
	background: #22C55E; color: #FFF; font-size: 22rpx; font-weight: bold; border-radius: 0 0 12rpx 0;
}
.item-info { padding: 20rpx 24rpx 24rpx; }
.item-title { font-size: 30rpx; font-weight: 600; color: #1A1D28; display: block; margin-bottom: 8rpx; }
.item-desc { font-size: 24rpx; color: #8B8FA3; line-height: 1.5; display: block; margin-bottom: 16rpx; }
.item-footer { display: flex; justify-content: space-between; align-items: center; }
.item-seller { display: flex; align-items: center; }
.item-avatar-sm { width: 40rpx; height: 40rpx; border-radius: 50%; margin-right: 10rpx; }
.item-seller-name { font-size: 24rpx; color: #6B6F80; }
.item-meta { display: flex; align-items: center; gap: 16rpx; }
.item-meta-item { display: flex; align-items: center; gap: 4rpx; }
.item-meta-item text, .item-location, .item-time { font-size: 22rpx; color: #B0B4C0; }
