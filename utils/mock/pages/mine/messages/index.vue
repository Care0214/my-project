	...
	<view class="page-container--clean">
		<view class="page-body">
			...
			<view v-if="showSearch" class="search-row">
				<input class="search-input" v-model="searchText" placeholder="搜索聊天记录..." confirm-type="search" />
			</view>

			<!-- 消息列表（按时间分组） -->
			<view v-if="filteredConversations.length > 0" class="msg-list">
				<view v-for="(group, gIdx) in groupedConversations" :key="gIdx">
					<view class="group-label"><text>{{ group.label }}</text></view>
		...
		groupedConversations() {
			...
			this.filteredConversations.forEach((c) => {
				...
				if (timeText.includes('分钟前') || timeText.includes('刚刚') || timeText.includes('小时前')) {
					...
					if (days <= 7) thisWeek.push(c); else earlier.push(c);
				} else {
					earlier.push(c);
				}
			});
...
.msg-avatar-wrap { position: relative; margin-right: 20rpx; flex-shrink: 0; }
.msg-avatar {
	width: 100rpx; height: 100rpx; border-radius: 50%;
	display: flex; align-items: center; justify-content: center;
}
.msg-avatar-text { font-size: 36rpx; font-weight: 600; color: #FFF; }
.online-dot {
	position: absolute; bottom: 2rpx; right: 2rpx; width: 22rpx; height: 22rpx;
...
.msg-name-row { flex: 1; min-width: 0; display: flex; flex-direction: column; gap: 6rpx; }
.msg-username { font-size: 30rpx; color: #1A1D28; font-weight: 600; }
.msg-item-tag {
